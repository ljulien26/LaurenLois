...
⚠️ **Actuellement forcé sur `fireworks` en DEBUG dans main.js** (bloc « démarrer directement sur… » à retirer pour rétablir le flux normal).
...
- **fireworks.js** : écran final (voir « Contenu »). Feux **procéduraux** (particules additives), fonds `assets.place8`, photos `assets.finalPhotos`, sons `fwMusic`/`fwFireSound`, message « Bon anniversaire » **écrit en particules**. **Bloc « arrivée du couple »** en tête du fichier : `fwLois`/`fwLauren`, `fwUpdateCouple`/`fwDrawCouple`, cœurs pixel (`FW_HEART_ROWS`). Repère décor via `getCoverTransform` (le fond est en **cover**), sol `FW_GROUND_Y=512`, centre `FW_COUPLE_X=480`, échelles `FW_LAUREN_SCALE=0.86` / `FW_LOIS_SCALE=1.0` / `FW_CLL_SCALE=1.03`. **Bruit de pas** (`updateWalkSound`) appelé à chaque image pendant l'arrivée.
...
- Écran final (fireworks) : **phase `arrivee` (non jouable)** — transition **lente** jour→coucher→nuit (`place8` = `Places/8/1-3.png` ; `FW_DAY_HOLD=1,5 s`, `FW_FADE=6 s`, `FW_SUNSET_HOLD=2 s` → nuit à **15,5 s**, fondus adoucis en smoothstep) pendant que **Loïs entre par la gauche et Lauren par la droite en même temps** (départ à `FW_COUPLE_START=0,7 s` **une fois l'audio débloqué**, sinon au plus tard `FW_COUPLE_MAX_WAIT=4 s` ; ~5 s de marche **avec le bruit de pas**), s'arrêtent **l'un contre l'autre** — ils marchent **directement** jusqu'à l'écart de l'étreinte (`FW_COUPLE_GAP=40`, mesuré sur le sprite CLL) et l'**étreinte** `CLL1→CLL4` enchaîne **aussitôt**, sans pose intermédiaire (les `Côté/Calin.png` ne servent plus que de pose d'arrêt, jamais affichée), en **fondu enchaîné** entre images (`FW_CLL_FRAME=420 ms`, `FW_CLL_BLEND=260 ms`) ; sur **CLL4** ils restent enlacés pour tout le reste de la scène avec de **petits cœurs pixel** qui s'envolent → phase **charge** « Clic, clic, clic ! » (`FW_TAP_GOAL=12`, **son de clic seulement pendant la charge**) → **spectacle** : `fwMusic` (**Every Breath You Take**) + `fwFireSound` (**11.Firework**, boucle) lancées ensemble après `FW_FIRE_DELAY=1.8 s` → **« Bon anniversaire ! »** en particules (`FW_MSG_AT=6 s`, durée `FW_MSG_DUR=6.5 s`) → **12 photos** (`finalPhotos`, mélangées, **chacune une fois**, zones réparties, `FW_PHOTO_SPAWN=4.2 s`, vie ~9–11 s) → après la dernière + `FW_END_DELAY=3.5 s` → **« Fin… »** (fondu noir ; musique à fond ~3.5 s puis fondu 4.5 s). Volumes : `fwMusic 0.5`, `FW_FIRE_VOLUME 0.3`.

## Assets
`Assets/Jeu/Places/1-7.png` (décors 960×540) ; `Places/8/1-3.png` (final jour/coucher/nuit) ; `Cadenas/`, `Ticket/1.png`, `Chat/Chat 1-3.png`, `Quiz/Boutons/`, `Panier/Panier.png`, `Puzzle/1.png`, `Bouton Suite/1.png`. `Assets/Persos/Lauren|Loïs/Côté/{Idle,Walk,Activer le bouton,Calin.png}`, `Lauren/Côté/WalkPanier/26-29.png`, `Assets/Persos/CLL1-4.png` (**couple enlacé**, écran final) et `TLL1-4.png` (couple main dans la main, **pas encore utilisé**). Sons `Assets/Sound/1.Walking … 9.Click.mp3`, `10.CadenasMolette.wav`, `11.Firework.mp3`, `Chat/Miaou.mp3`, `Générique/` (Take on Me = menu ; Every Breath You Take = écran final ; copyright — le dev a choisi de les pousser). `Assets/Photo Fin de Jeu/1-12.png` (**utilisées** dans l'écran final).
...
- **DEBUG (à retirer pour la version finale)** :
  ...
  - **main.js force le démarrage sur `fireworks`** (bloc DEBUG) — à retirer pour repartir de `premenu`.
...
- **TODO** : suite de l'écran final (le couple est pour l'instant **de côté** ; des persos **de dos** regardant le feu restent à faire si souhaité) ; retirer les raccourcis DEBUG Maj+3…7 et le démarrage forcé pour la version cadeau finale.
